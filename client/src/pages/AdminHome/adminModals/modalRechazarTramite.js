...
const showModalRechazar = (tramiteId) => {
  let timerInterval;
  ...
  Swal.fire({
    html: `
      <div class="custom-alert-content">
        ...
        <div class="contenedor-titulo-alert"><h2 class="titulo-alert">¿Estás seguro que quieres rechazar este trámite?</h2></div>
    ...
    didOpen: () => {
      ...
      modalContainer.querySelector('#btn-si').addEventListener('click', async () => {
        try {
          await fetch(`http://localhost:3000/api/gestionartramite/${tramiteId}/rechazar`, {
            method: 'PUT',
          });
          Swal.close();
          window.location.reload();
        } catch (error) {
          console.error('Error al rechazar el trámite:', error);
        }
      });
...
export default showModalRechazar;
